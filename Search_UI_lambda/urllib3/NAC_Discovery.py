...
def lambda_handler(event, context):
    ...
    # secret_data_internal['nac-stack'] = ''
    ...
    for record in event['Records']:
        ...
        # data['volume_name'] = split_files_n_folders[len(split_files_n_folders) - 2]
        
        secret_data_prod = get_secret('prod/nac/user',data['awsRegion']) 
        secret_data_internal = get_secret('nac-es-internal',data['awsRegion'])
        
        data['volume_name'] = secret_data_prod['volume_name']
        data['root_handle'] = secret_data_prod['root-handle'].replace('.','_').lower()
        data['source_bucket'] = secret_data_prod['source_bucket']
        ...
        # index = create_index_name(data) 
        
        
        data['access_url']='https://'+secret_data_prod['web_access_appliance_address']+'/fs/view/'+data['object_key']
        print('data',data)
        print('secret_data_prod',secret_data_prod)
        print('secret_data_internal',secret_data_internal)
        es_obj = launch_es(secret_data_internal['es-url'],data['awsRegion'])
 
        # add_role_mapping(es_obj)
        connect_es(es_obj,data['root_handle'], data)
        # del_cloudformation_stack(secret_data_internal['nac-stack'])
        stack_name=secret_data_internal['nac-stack']
...
def add_role_mapping(es):
    body={
        "backend_roles" : ["arn:aws:iam::514960042727:role/nct-NCE-lambda-lambda_exec_role-NAC_Discovery-769383c5e910" ],
        "hosts" : [],
        "users" : [ "automation","Sumeet" ]
    }
    role_name='all_access'
    response=es.put_role_mapping(role_name,name, body) 
    print('put_role_mapping=',response)
